use {
    copernica::{
        InterLinkPacket, Link, NarrowWaist
    },
...
pub fn decode(msg: Vec<u8>) -> Result<NarrowWaist> {
    ...
    let nw = NarrowWaist::try_from_slice(&reconstituted[..])?;
    Ok(nw)
}

pub fn encode(ilp: InterLinkPacket) -> Result<Vec<u8>> {
    let mut merged = vec![];
    let enc = Encoder::new(12);
    let nw = ilp.narrow_waist().try_to_vec()?;
